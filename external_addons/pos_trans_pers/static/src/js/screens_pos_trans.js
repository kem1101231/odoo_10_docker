odoo.define('pos_trans_pers.screens_pos_trans', function (require) {
"use strict";
...
var models = require('pos_trans_pers.models_pos_trans');
console.log(models)
console.log(models.PosDB)
var core = require('web.core');
var Model = require('web.DataModel');
var utils = require('web.utils');
var formats = require('web.formats');

//var mod_pos = require('pos_trans_pers.mod_test')
var custom_model = new Model('pos.order.personel')
...
localStorage.setItem("pos_trans", JSON.stringify({}))
var offLineData = {};

var currentData = { 'mech_id':null,
                    'omec_id':null,
                    'chkr_id':null,
                    'pckr_id':null,
                    'ord_id':null
...
var ScreenWidget = PosBaseWidget.extend({
    ...
    init: function(parent,options){
       ...
       // this.displayData('mech');
       // this.displayData('chkr');
       // this.displayData('pckr');
    },
    ...
    renderElement: function() {
        ...
        this.$('.pay').click(function(){
            ...
            if(!has_valid_product_lot){
            	...
            	if (mech_sel_id != null) {
            		$('.js_mecha_name').text(mech_sel_id['name'])
            	}
            	if (omec_sel_id != null) {
            		$('.js_omech_name').text(omec_sel_id['name'])
            	}
            	if (chkr_sel_id != null) {
            		$('.js_chckr_name').text(chkr_sel_id['name'])
            	}
            	if (pckr_sel_id != null) {
            		$('.js_packr_name').text(pckr_sel_id['name'])
            	}
...
var MechaListScreenWidget = genScreenWidget('MechaListScreenWidget', 'mecha')
gui.define_screen({name:'mechalist', widget: MechaListScreenWidget});

var OMechListScreenWidget = genScreenWidget('OMechListScreenWidget', 'omech')
gui.define_screen({name:'omechlist', widget: OMechListScreenWidget});

var ChckrListScreenWidget = genScreenWidget('ChckrListScreenWidget', 'chckr')
gui.define_screen({name:'chckrlist', widget: ChckrListScreenWidget});

var PackrListScreenWidget = genScreenWidget('PackrListScreenWidget', 'packr')
gui.define_screen({name:'packrlist', widget: PackrListScreenWidget});

var btnRefDataU = {'mecha':mech_sel_id,'omech':omec_sel_id,'chckr':chkr_sel_id,'packr':pckr_sel_id};

function genScreenWidget(widgetTemplate, scrType){
  var btnString = {'mecha':'Mechanic','omech':'Outside Mechanic','chckr':'Checker','packr':'Packer'}
  var btnRefData = {'mecha':mech_sel_id,'omech':omec_sel_id, 'chckr':chkr_sel_id,'packr':pckr_sel_id}
  var widgetOutput = ScreenWidget.extend({
        ...
        show: function(){
            ...
            this.$('.next').click(function(){
                ...
                switch(scrType){
                    case 'mecha':
                        if(mech_sel_id != null && mech_sel_id['id'] == temp_sel_id['id']){
                                
                                mech_sel_id = null
                                currentData['mech_id'] = null
                                self.pos.set_mech_sel_id = null

                                $('.js_mecha_name').removeAttr('style')
                                $('.js_mecha_name').text('Mechanic')
                                btnRefData[scrType] = null
                        }
                        else{
                               
                                mech_sel_id = temp_sel_id
                                currentData['mech_id'] = mech_sel_id['id']
                                var name = mech_sel_id['name']
                                self.pos.set_mech_sel_id = mech_sel_id['name']

                                if(name.length > 32){
                                    $('.js_mecha_name').attr('style','font-size:11px;')
                                }
                                if(name.length > 20){
                                    $('.js_mecha_name').attr('style','font-size:12px;')
                                }
                                if(name.length < 20){
                                    $('.js_mecha_name').removeAttr('style')
                                }
                                ...
                                btnRefData[scrType] = temp_sel_id
                        }

                        break;

                   case 'omech':
                        if(omec_sel_id != null && omec_sel_id['id'] == temp_sel_id['id']){
                               
                                omec_sel_id = null
                                currentData['omec_id'] = null
                                self.pos.set_omec_sel_id = null

                                $('.js_omech_name').removeAttr('style')

                                $('.js_omech_name').text('Outside Mechanic')
                                btnRefData[scrType] = null
                        }
                        else{
                                
                                omec_sel_id = temp_sel_id
                                currentData['omec_id'] = omec_sel_id['id']
                                var name = omec_sel_id['name']
                                self.pos.set_omec_sel_id = omec_sel_id['name']

                                if(name.length > 32){
                                    $('.js_omech_name').attr('style','font-size:11px;')
                                }
                                if(name.length > 20){
                                    $('.js_omech_name').attr('style','font-size:12px;')
                                }
                                if(name.length < 20){
                                    $('.js_omech_name').removeAttr('style')
                                }
                    ...
                    case 'chckr':
                        ...
                        if(chkr_sel_id != null && chkr_sel_id['id'] == temp_sel_id['id']){
                               
                                chkr_sel_id = null
                                currentData['chkr_id'] = chkr_sel_id['id']
                                self.pos.set_chkr_sel_id = null

                                $('.js_chckr_name').removeAttr('style')

                                $('.js_chckr_name').text('Checker')
                                btnRefData[scrType] = null
                        }
                        else{
                               
                                chkr_sel_id = temp_sel_id
                                currentData['chkr_id'] = chkr_sel_id['id']
                                var name = chkr_sel_id['name']
                                self.pos.set_chkr_sel_id = chkr_sel_id['name']

                                if(name.length > 32){
                                    $('.js_chckr_name').attr('style','font-size:11px;')
                                }
                                if(name.length > 20){
                                    $('.js_chckr_name').attr('style','font-size:12px;')
                                }
                                if(name.length < 20){
                                    $('.js_chckr_name').removeAttr('style')
                                }
                    ...
                    case 'packr':
                        ...
                        if(pckr_sel_id != null && pckr_sel_id['id'] == temp_sel_id['id']){
                               
                                pckr_sel_id = null
                                currentData['pckr_id'] = pckr_sel_id['id']
                                self.pos.set_pckr_sel_id = null
                                $('.js_packr_name').removeAttr('style')

                                $('.js_packr_name').text('Packer')
                                btnRefData[scrType] = null
                        }
                        else{
                                pckr_sel_id = temp_sel_id
                                currentData['pckr_id'] = pckr_sel_id['id']
                                var name = pckr_sel_id['name']
                                self.pos.set_pckr_sel_id = pckr_sel_id['name']

                                if(name.length > 32){
                                    $('.js_packr_name').attr('style','font-size:11px;')
                                }
                                if(name.length > 20){
                                    $('.js_packr_name').attr('style','font-size:12px;')
                                }
                                if(name.length < 20){
                                    $('.js_packr_name').removeAttr('style')
                                }
            ...
            if( btnRefData[scrType] ){
                console.log(this.old_client)
                console.log(btnRefData[scrType])
                this.display_client_details('show',btnRefData[scrType],0);
            }
        ...
        render_list: function(partners){
            var contents = this.$el[0].querySelector('.client-list-contents');
            var refdata = btnRefData[scrType]
            var refDataID;

            if(refdata != null){
                refDataID = refdata['id']
            }
            contents.innerHTML = "";
            for(var i = 0, len = Math.min(partners.length,1000); i < len; i++){
                var partner    = partners[i];
                ...
                if( refdata!= null && partner.id === refdata['id']){
                    clientline.classList.add('highlight');
                }else{
                    clientline.classList.remove('highlight');
                }
        ...
        toggle_save_button: function(){
            ...
            var refDataN = btnRefData[scrType]

            var $button = this.$('.button.next');
            if (this.editing_client) {
                $button.addClass('oe_hidden');
                return;
            } else if(refDataN == null || refDataN['id'] != temp_sel_id['id']){
                if(refDataN == null){
                    $button.text(_t('Set '+ btnString[scrType]));
                }else{
                    $button.text(_t('Change '+ btnString[scrType]));
                }
            }else{
                $button.text(_t('Deselect '+ btnString[scrType]));
            }
            $button.toggleClass('oe_hidden',!this.has_client_changed());
        },
        line_select: function(event,$line,id){
...
var ReceiptScreenWidget = ScreenWidget.extend({
    ...
    print_web: function() {
        ...
        //custom_model.call('get_pos_trans_data', {'ord_id': currentData['ord_id']})
    },
    print_xml: function() {
        var env = {
            widget:  this,
    ...
    render_receipt: function() {
       ...
       //custom_model.call('get_pos_trans_data', {'ord_id': 1})
    },
});
...
var PaymentScreenWidget = ScreenWidget.extend({
    ...
    mecha_changed: function() {
        var client = this.pos.get_client();
        this.$('.js_mecha_name').text(mech_sel_id['name']);
    },
    click_set_customer: function(){
        this.gui.show_screen('clientlist');
    },
    click_set_mecha: function(){
    	
        if(mech_sel_id != null){
            temp_sel_id = mech_sel_id
        }
        this.gui.show_screen('mechalist');
    },
    click_set_omech: function(){

        if(omec_sel_id != null){

            temp_sel_id = omec_sel_id
        }
        this.gui.show_screen('omechlist');
    },
    click_set_chckr: function(){

        if(chkr_sel_id != null){

            temp_sel_id = chkr_sel_id
        }
        this.gui.show_screen('chckrlist');
    },
    click_set_packr: function(){

        if(pckr_sel_id != null){

            temp_sel_id = pckr_sel_id
        }
        this.gui.show_screen('packrlist');
    },
    click_back: function(){
    ...
    renderElement: function() {
        ...
        this.$('.js_set_mecha').click(function(){
            self.click_set_mecha();
        });
        this.$('.js_set_omech').click(function(){
            self.click_set_omech();
        });
        this.$('.js_set_chckr').click(function(){
            self.click_set_chckr();
        });
        this.$('.js_set_packr').click(function(){
            self.click_set_packr();
        });
    ...
    finalize_validation: function() {
        ...
        console.log("Calling finalized validation")
        ...
        //self.addPOSTransData(order.uid);
        
    },

    addPOSTransData: function(orderid){

        currentData['ord_id'] = orderid
        if (mech_sel_id != undefined) {
            currentData['mech_id'] = mech_sel_id['id'];
        }
        if (omec_sel_id != undefined) {
            currentData['omec_id'] = omec_sel_id['id'];
        }
        if (chkr_sel_id != undefined) {
            currentData['chkr_id'] = chkr_sel_id['id'];
        }
        if (pckr_sel_id != undefined) {
            currentData['pckr_id'] = pckr_sel_id['id'];
        }
        ...
        var conn_state = custom_model.call('testConnection')

        conn_state.fail(function(e) {
            offLineData[order.uid] = {  'mech_id': mech_sel_id['id'],
                                        'omec_id': omec_sel_id['id'], 
                                        'chkr_id': chkr_sel_id['id'], 
                                        'pckr_id': pckr_sel_id['id']
                                    }
            console.log("Done Saving Locally")
            localStorage.setItem("pos_trans", JSON.stringify(offLineData));
            console.log(JSON.parse(localStorage.getItem("pos_trans")))                
        })

        conn_state.done(function(e) {  
            var pos_trans = JSON.parse(localStorage.getItem("pos_trans"))
            if (pos_trans != {}) {
                
                custom_model.call('massAddData', {'dataList':JSON.stringify(pos_trans),})
            }

            custom_model.call('addData', {'dataList':JSON.stringify(currentData),})
    ...
    validate_order: function(force_validation) {
       ...
       if (this.order_is_valid(force_validation)) {
            ...
            this.addPOSTransData(order.uid);
            custom_model.call('get_pos_trans_data', {'ord_id': currentData['ord_id']})
        }
    },
});
